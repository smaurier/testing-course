// =============================================================================
// Lab 17 — Performance Testing Patterns (exercise)
// =============================================================================
// Instructions : implementez chaque fonction TODO puis lancez les tests.
// Commande : npx tsx labs/lab-17-performance/exercise.ts
// =============================================================================
...
export async function benchmark(
  ...
  // TODO: Executer fn `iterations` fois et mesurer les temps
  // - Calculer avg, min, max, p95 (95e percentile)
  // - p95: trier les durees, prendre l'element a l'index Math.ceil(0.95 * n) - 1
...
export async function loadTest(
  ...
  // TODO: Lancer `concurrency` workers en parallele pendant `durationMs` ms
  // - Chaque worker execute fn en boucle jusqu'a expiration du temps
  // - Compter succes et echecs
  // - Calculer avgResponseTime et requestsPerSecond
  throw new Error('Not implemented');
}
...
export async function throughputTest(
  ...
  // TODO: Lancer des requetes a un rythme cible (targetRPS)
  // - Espacer les requetes de 1000/targetRPS ms
  // - Mesurer le debit reel atteint
  throw new Error('Not implemented');
}
...
export async function memoryProfile(
  ...
  // TODO: Mesurer le heap avant et apres l'execution
  // - Executer fn `iterations` fois
  // - Comparer initialHeap et finalHeap
...
export function regressionDetect(
  ...
  // TODO: Pour chaque metrique, comparer baseline et current
  // - Calculer le changement en % ((current - baseline) / baseline * 100)
  // - regressed = changePercent > thresholdPercent (pour les metriques de temps,
  //   une augmentation est une regression)
  throw new Error('Not implemented');
}
...
export async function runPerformanceSuite(config: {
  ...
  // TODO: Assembler tous les tests de performance
  // - benchmark + load test + memory profile
  // - Si baseline fournie, detecter les regressions
  // - passed = pas de fuites memoire ET pas de regressions
  throw new Error('Not implemented');
}
...
async function main() {
  ...
  await test('Ex5: regressionDetect identifies regressions', () => {
    ...
    assertEqual(p95Result.regressed, false); // 5% < 20%
  });
  ...
  await test('Ex6: full performance suite runs all checks', async () => {
    ...
    assertEqual(result.regressions.length, 0); // no baseline
  ...
  await test('Ex6: full suite detects regression with baseline', async () => {
    const result = await runPerformanceSuite({
      ...
      loadConcurrency: 1,
      loadDurationMs: 50,
      baseline: { avg: 1 }, // baseline says 1ms but fn takes 5ms
    });
