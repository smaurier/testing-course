...
interface MockHandler {
  method: HttpMethod;
  pattern: string;
  handler: (req: { body?: unknown; params?: Record<string, string> }) => MockResponse;
}
...
function createMockServer() {
  ...
  function fetch(
    url: string,
    options?: { method?: HttpMethod; body?: string; headers?: Record<string, string> }
  ): MockResponse {
...
await test('Ex2: GET /api/users retourne la liste', () => {
  ...
  assertEqual(res.status, 200);
  assertEqual((res.body as unknown[]).length, 2);
});
...
await test('Ex4: path params /api/users/:id', () => {
  ...
  assertEqual((res1.body as { name: string }).name, 'Alice');
...
await test('Ex6: CRUD complet — create, read, update, delete', () => {
  ...
  // Create
  const created = server.fetch('/api/users', {
    method: 'POST',
    body: JSON.stringify({ name: 'Alice', email: 'alice@test.com' }),
  });
  ...
  assertEqual((read.body as { name: string }).name, 'Alice');

  // Update
  const updated = server.fetch(`/api/users/${userId}`, {
    method: 'PUT',
    body: JSON.stringify({ name: 'Alice Updated' }),
  });
  assertEqual(updated.status, 200);
  assertEqual((updated.body as { name: string }).name, 'Alice Updated');
