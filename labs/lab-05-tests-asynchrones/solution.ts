...
function debounce<T extends (...args: any[]) => void>(fn: T, delayMs: number): T & { cancel: () => void } {
...
await test('Ex3: pubsub delivers messages to subscribers', () => {
  ...
  let received = '';
  ps.subscribe('greeting', (data) => { received = data; });
...
await test('Ex3: pubsub unsubscribe stops delivery', () => {
  const ps = new PubSub<string>();
  let count = 0;
  const unsub = ps.subscribe('evt', () => { count++; });
  ps.publish('evt', 'a');
  assertEqual(count, 1);
  unsub();
  ps.publish('evt', 'b');
...
await test('Ex3: pubsub different events are isolated', () => {
  ...
  let aReceived = false;
  let bReceived = false;
  ps.subscribe('a', () => { aReceived = true; });
  ps.subscribe('b', () => { bReceived = true; });
  ps.publish('a', 'x');
  assert(aReceived);
  assert(!bReceived);
});
...
await test('Ex4: debounce passes arguments to original function', async () => {
  let lastArg = '';
  const fn = debounce((msg: string) => { lastArg = msg; }, 30);
  fn('first');
  fn('second');
  fn('third');
  await delay(80);
...
await test('Ex6: unsafe counter may lose increments under concurrency', async () => {
  ...
  assert(counter.getCount() <= 3, `Count should be <= 3, got ${counter.getCount()}`);
});
